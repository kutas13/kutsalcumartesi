...
export async function readState(){
  ...
  // Fresh SQL databases return empty arrays. Seed the four core accounts once.
  if(!Array.isArray(state.accounts) || state.accounts.length===0){
    const seeded={...base,version:7};
    const {data:seededData,error:seedError}=await supabaseAdmin.rpc('kck_replace_state',{p_state:seeded});
    if(seedError) throw seedError;
    return {...base,...(seededData||seeded),version:7};
  }

  return {
    ...base,
    ...state,
    accounts:Array.isArray(state.accounts)?state.accounts:base.accounts,
    ...
    prices:state.prices&&typeof state.prices==='object'?state.prices:base.prices,
    priceNames:state.priceNames&&typeof state.priceNames==='object'?state.priceNames:base.priceNames,
